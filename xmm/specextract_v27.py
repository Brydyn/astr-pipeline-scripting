...
Purpose:
Automate XMM-Newton extended source spectral extraction with region exclusion, background handling,
...
for n in range(camNum):
    ...
    for s in range(start, stop):
        ...
        try:
            ...
            os.system(f"rmfgen rmfset={rmf} spectrumset={spectrum} detmaptype=flat")
            os.system(f"arfgen arfset={arf} spectrumset={spectrum} withrmfset=yes rmfset={rmf} detmaptype=flat extendedsource=yes withbadpixcorr=yes badpixlocation={filePath[n]}")
        except Exception as e:
